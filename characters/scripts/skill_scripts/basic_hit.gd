...
func _process(delta: float) -> void:
	if selected:
		...
		var dir = mouse_pos - char_pos
		
		# Determinar si predomina horizontal o vertical
		if abs(dir.x) > abs(dir.y):
			# Horizontal
			if dir.x > 0:
				global_position = char_pos + Vector2(offset, 0) # derecha
			else:
				global_position = char_pos + Vector2(-offset, 0) # izquierda
		else:
			# Vertical
			if dir.y > 0:
				global_position = char_pos + Vector2(0, offset) # abajo
			else:
				global_position = char_pos + Vector2(0, -offset) # arriba
